...
> **⚠ 代码状态声明**：以下所有 `src/` 路径为规划目标，非现有代码。截至 v1.0.0 草稿完善阶段，全系统尚无运行代码（见 changelog/project-plan）。编码启动后按此映射创建模块。
...
| 版本 | 日期 | 变更 |
|:----|:----|:-----|
| v1.0.0 | 2026-07-20 | 初始实现映射。六层架构 + 横切 + 测试，约 64 个组件映射到 src/ 模块路径。 |
