...
| 外部治理接口 | 外部通信层 | 接收外部校准信号、冻结/解冻指令；输出审计日志供外部读取。不持有至高否决权——策略层在执行决策前查询本接口状态自行决定。由原宪法主权面与监督平面合并（见 §1.7） |
|（已合并入外部治理接口 §1.7） | — | — |
...
| 注意力调度器 | 独立横切服务 | 统一分配编码、巩固、检索三环节的算力预算。通过事件总线接收资源请求，通过注册表查询当前资源池状态，通过事件总线返回分配结果。WM、存储层、策略层均为客户端——调度器本身不驻留任何层（见 §9 注意力调度器声明）。元约束面的执行实体（与外部治理接口并行） |
...
**推论冲突裁决：** 探索 > 宪法 > 校准 > 认知完整性 > 时间 > 间接度（六级链，与认知基础§2.1一致）。身份不在此链中——身份是独立的正交治理面（身份面），以否决权（而非排名）介入排序产出。裁决器使用**值优先**比较（宪法 > 校准 = 宪法权重大于校准）。探索→宪法为**并行审查模型**——探索候选产生后不立即执行，进入短暂的宪法审查窗口（可配置，默认 100ms），审查通过后执行。窗口超时未获审查结果，默认拒绝执行（fail-close）。此模型确保安全红线不会被绕过，同时探索不被事前审批阻塞（审查是并行的）。原「时序优先」概念已删除。六级链的逐对论证见认知基础§2.1及架构§3.2辞典式裁决器。身份面否决权与宪法主权面否决权的区别见认知基础§2.1。
...
**统一口径**：全文档集以「六层+正交面」为默认计数——五层功能栈（接入/WM/存储/策略/元认知）+ 宪法主权面（第 6 结构单元）。监督平面是正交审计面，不计入层数。
...
> 推理皮层（§4）为独立协调层，不入功能栈层编号。注意力调度器（§9）为独立横切服务，不入功能栈层编号。注意力调度器不物理驻留任何功能层——它通过事件总线接收资源请求，经注册表查询资源池状态后返回分配结果。WM、存储层、策略层均为其客户端。此独立性确保调度器不会因驻留位置而产生资源分配偏向。
>
...
| WM 层 | 当前任务处理 + 未来预演；推理皮层（§4）——**推理皮层不是"推理引擎"，而是"调度预处理器"**。其唯一职能：(1) 对候选记忆进行快速排序（基于已计算的权重，不调用外部模型）；(2) 对触发条件做模式匹配（基于规则而非推理）；(3) 对候选集做上下文裁剪（基于路径前缀和 TTL）。禁止承载：通用语义理解、跨任务规划、生成新记忆内容。三类操作若需要，归属外部应用层。推理皮层物理驻留 WM（见 §0.4 六类结构原则说明） | 不持久化（临时隔离区） |
...
```
  ...
  │   ├─ 上下文腐烂指数监测器（CRI Monitor）——综合注意力分布熵、有效信息利用率和任务成功率三项运行指标，根据认知基础§1.9定义的三阶段模型，输出 [0,1] 区间的 CRI 值。CRI 超阈值（默认 0.6）时触发：① 通知策略层降级检索深度为浅层（参见检索深度分级 §3.2）；② 通知编译器提升 L2 解析的严格度（减少模糊容忍）；③ 向宪法解释层提交「上下文腐烂告警」，作为身份面否决权介入的参考条件。CRI 计算为轻量离线任务（每调度周期扫一次，不参与实时推理路径）。
...
|:-------|:---------|:---------|:---------|:------------|
| **RL0 浅层** | 简单参数查询、状态确认、CRI 超阈时强制降级 | 路径注册表前缀匹配 + 索引元数据过滤，不触发向量计算 | 记忆标题/标签/路径/路径密度 | ~0.05× |
| **RL1 中层** | 中等复杂度信息确认、跨域关联探索 | 摘要级向量相似度（嵌入限于 128 维摘要向量）+ BM25 全文匹配 | 记忆摘要（content_summary 字段）+ 来源元数据 | ~0.3× |
| **RL2 全面** | 深度分析、多步推理、精确事实比对 | 全量 2048 维语义向量匹配 + 全内容重建 + cross-encoder 重排序 | 完整记忆内容 + 多维得分明细 | 1× |

深度选择由编译器在生成结构化通信单元时根据任务复杂度标记。运行时策略层可依据当前 CRI 值主动降级：CRI > 0.6 时强制降为 RL0 浅层，CRI > 0.4 时降为 RL1 中层。降级触发器来自元认知层 CRI 监测器（§2.2），经事件总线通知策略层路由组件。

> **CRI 阈值说明**：检索深度降级（§3.9 RL0/RL1）与决策模式（§3.10 L0-L3）是**两个独立的并行机制**——前者控制检索计算成本（检索多深），后者控制输出自主度（谁做决定）。两者同时对 CRI 响应但阈值不同：检索深度在 CRI>0.4/0.6 降级，决策模式在 0.3/0.5/0.7 切换。在 CRI∈[0.6,0.7) 重叠区间，系统同时处于 L2（多方案推荐）和 RL0（浅层检索），属正常状态——低成本检索结果仍可输出多方案比较。
...
> **占位声明**（v1.0 范围外）：本节定义多 Agent 校准参数的接口契约。当前版本仅声明三个必含维度（来源独立性、历史准确率、交叉印证度）和否决权归属（§1.7 §11），具体实现推迟至 v1.1+。相关路径空间 `kairos://_social/` 已预留（见 §1.2 预留路径）。
...
**否决权**：多 Agent 校准的激活开关归属宪法主权面（§1.7），系统在激活前仅使用单 Agent 校准模型。激活前提条件（见 §1.2 占位声明）全部满足后方可开启。此占位不约束具体存储结构或算法，仅声明上述接口契约。
...
**实体加成（Entity Boost）**：作为可选的第 6 扩展维度，在基础 5D 混合排序（语义+BM25+时序+信任+热度）之上叠加。对查询执行实体识别，得到查询实体集合 Q → 对每个候选结果 R，计算 R 关联实体与 Q 的交集比例 n/|Q| → 实体加成得分 = min(1 + n × KAIROS_ENTITY_BOOST_PER_MATCH(0.10), KAIROS_ENTITY_BOOST_MAX(1.5))。权重 w_e = KAIROS_ENTITY_BOOST_WEIGHT(0.10)，归一化后各维度权重和为 1。

> **术语说明**：本架构以「5D 混合排序」指代基础检索维度（语义+BM25+时序+信任+热度），entity_boost 与图谱距离为可选自适应扩展信号——激活后实际为 6D/7D 排序。基础维度数在 v1.0 中固定为 5，扩展信号可独立启用/关闭，不改变基础维度的计数口径。
...
> **占位声明**：本节全部指标值为架构层理论推导，标记为设计目标。当前标注「≥」「≤」「<」等阈值均为 v1.0 占位值，未经代码运行验证。代码启动后首个迭代执行基准测试，验证通过后晋升为已验收指标。§10.6–§10.15 的认知质量指标同理。

| 公理/推论 | 指标 | 占位值（v1.0 设计目标，待代码验证后定标） |
|:---------|:----|:-----------|
...
P6 全局闸门规定累计压缩比上限为 30%。v1.0 的活跃 P6 例外中，压缩维度占比需区分两类：**主动压缩**（负载系数、VAD 维度降维等直接降低信息维度的操作）占比 ~33%（约 12 维基数）；**被动压缩**（探索预算乘法、预算封顶等非降维的预算分配机制，不直接降低信息维度）占比 ~43%（含主动+被动共四类例外，约 14 维基数）。主动压缩 ~33% 已超出 P6 的 30% 上限，当前状态为受控偏离（详见 §3.2 §0.6）。被动压缩不直接违背 P6 字面约束，但需在偏离监控中跟踪累计影响。为监控偏离不继续扩大并规划恢复路径，建立以下监控规则：
...
| 遗忘调度器 | 单曲线指数衰减（v1.0 实现；理想设计为二维遗忘曲面） | 停用遗忘，仅标记待归档 | skip_forgetting —— 仅标记、不处理遗忘候选 |
...
| **检索深度分级** | RL0（浅层/路径匹配）→RL1（中层/摘要匹配）→RL2（全面/全量重建）三级，受 CRI 降级控制 |
...
| 版本 | 日期 | 说明 |
|:----|:----|:-----|
| v1.0.0 | 2026-07-23 | 架构设计定稿（文档仍保持 draft 状态——无运行代码验证）。完整定义六层功能栈（接入/WM/存储/策略/元认知/宪法主权面）+ 监督平面正交治理结构。核心设计：双副本分离（见证锚定主副本 + 使用权重影子副本）、路径空间（kairos:// 分层寻址）、四类使用契约模型、五条推论、P1–P6 设计原则约束、六级辞典式裁决器+身份面否决权、三环不变量。存储层：三区域知识生产模型（加工/验证/正式库）、关系索引、多维特征空间、升华管道、遗忘调度器方案。扩展组件：接入层编译器（L1规则+L2模型，结构化通信单元输出）；注册表（五根键域确定性状态存储）；元认知层 CRI 监测器与热纠偏监测器（已重构——CRI阈值见§3.9/§3.10「受控待统一」）。扩展机制：解决方案谱系管理、多级蒸馏结构（碎片/会话/日/体系/元规则）、知识灭绝与化石机制、MTL 逻辑-物理地址分离。安全红线 19 条（S-01~S-19）。质量属性 20+ 项量化指标。部署梯度（内核/标准/全量三级）。
