...
| 版本 | 日期 | 说明 |
|:----|:----|:-----|
| v1.0.0 | 2026-07-23 | 架构首版定稿。完整定义六层功能栈（接入/WM/存储/策略/元认知/宪法主权面）+ 监督平面正交治理结构。核心设计：双副本分离（见证锚定主副本 + 使用权重影子副本）、路径空间（kairos:// 分层寻址）、四类使用契约模型、五条推论、P1–P6 设计原则约束、六级辞典式裁决器+身份面否决权、真理模式切换协议、三环不变量。存储层：三区域知识生产模型（加工/验证/正式库）、关系索引、多维特征空间、升华管道、遗忘调度器方案。扩展组件：接入层编译器（L1规则+L2模型，结构化通信单元输出）；注册表（五根键域确定性状态存储）；元认知层 CRI 监测器与热纠偏监测器。扩展机制：解决方案谱系管理、多级蒸馏结构（碎片/会话/日/体系/元规则）、知识灭绝与化石机制、MTL 逻辑-物理地址分离。安全红线 20 条。质量属性 20+ 项量化指标。部署梯度（内核/标准/全量三级）。
