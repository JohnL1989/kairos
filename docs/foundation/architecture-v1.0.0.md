...
系统架构建立在认知基础文档的五轴度量空间上。**认知层→架构 v1.0 信息损失率统计**：五轴中仅三轴（使用/见证/时间）为完整连续度量，认知完整性轴为布尔 is_structure，可及性轴为工程代理——完整信息损失约 40%。六级链中「认知完整性 > 时间」的位次在 v1.0 以布尔标记参与，实际排序参与度低于规范声明。v1.0 以四轴承载（使用价值/见证价值/时间/认知完整性），第五轴（可及性轴）以路径竞争降权幅度和路径密度监测的联合指标作为工程代理，认知层完整度量待 v1.1 实现。
...
**两部接力（带常驻硬约束）：** 候选生成阶段使用带硬约束的多维帕累托排序——使用价值/见证价值/时间为主排序维度，探索/宪法/校准为硬约束维度始终参与计算（权重低于使用价值）。此设计使六级链中的高优先级维度在日常调度中持续生效，而非仅在帕累托平局时兜底介入。约束维度默认各为使用价值权重的 0.3（宪法 > 校准 > 探索），可配置。排序产出保留处于帕累托前沿的不可支配解集；冲突裁决仅在不可支配解之间以辞典式排序做兜底（不再假设目标可比）。两部接力而非矛盾。
...
**推论冲突裁决：** 探索 > 宪法 > 校准 > 认知完整性 > 时间 > 间接度（六级链，与认知基础§2.1一致）。身份不在此链中——身份是独立的正交治理面（身份面），以否决权（而非排名）介入排序产出。裁决器使用**值优先**比较（宪法 > 校准 = 宪法权重大于校准）。探索→宪法为**并行审查模型**——探索候选产生后不立即执行，进入短暂的宪法审查窗口（可配置，默认 100ms），审查通过后执行。窗口超时未获审查结果，默认拒绝执行（fail-close）。此模型确保安全红线不会被绕过，同时探索不被事前审批阻塞（审查是并行的）。原「时序优先」概念已删除。**注意**：六级链在 v1.0 中是建议性基线——宪法核的不可触及性受限于外部信令不发出冲突指令。所有优先级均可被外部校准信号覆盖。六级链的逐对论证见认知基础§2.1及架构§3.2辞典式裁决器。身份面否决权与宪法主权面否决权的区别见认知基础§2.1。
...
```
  ...
  **层划分的推导判据声明**：当前五层功能栈（接入/WM/存储/策略/元认知）+ 宪法主权面 + 监督平面（合计 7 个结构单元）的层数不是约定俗成的工程惯例，而是从认知基础推导而来——每一层对应一个不可再分的认知职能族：(1) 接入层 = 感知入口（不可再分：任何记忆系统都需要交互边界）；(2) WM 层 = 当前认知活跃区（不可再分：工作区与持久存储的分离是记忆系统的基本结构）；(3) 存储层 = 持久表征（不可再分：长时记忆是记忆系统的本质）；(4) 策略层 = 使用决策（不可再分：记忆的使用策略必须独立于存储机制，P3 推论）；(5) 元认知层 = 自我认知（不可再分：反身性是上限自反性的最小承载）。宪法主权面与监督平面是正交治理面，**监督平面为代码路径独立的审计监视器——独立性为声明级（代码路径独立）而非数据源级（数据源依赖存储层只读接口）——真正的独立审计者是人类操作员**。不属于功能栈层数。**计数说明**：本文「六层」将宪法主权面计入层数（§0.5 一致性表 L157-164 列出 6 个结构单元包括宪法主权面），「五层功能栈」排除正交平面——两套计数不矛盾，差异源于宪法主权面视为功能层还是正交治理面。此推导判据确保层基数可被追溯而非仅继承自工程经验。
...
> 推理皮层（§4）为独立协调层，不入功能栈层编号。注意力调度器（§9）为独立横切服务，不入功能栈层编号。注意力调度器不物理驻留任何功能层——它通过事件总线接收资源请求，经注册表查询资源池状态后返回分配结果。WM、存储层、策略层均为其客户端。此独立性确保调度器不会因驻留位置而产生资源分配偏向。**实现指引：v1.0 组件边界是认知驱动的设计假设——实现阶段可基于运行经验合并或简化。预期约 30% 的组件可能在首次实现中被合并（特别是治理面相关的辅助组件），合并后登记于 debt-collection.md，不触发「违反架构」审查**
>
...
| WM 层 | 当前任务处理 + 未来预演；推理皮层（§4）——**推理皮层不是"推理引擎"，而是"调度预处理器"**。其唯一职能：(1) 对候选记忆进行快速排序（基于已计算的权重，不调用外部模型）；(2) 对触发条件做模式匹配（基于规则而非推理）；(3) 对候选集做上下文裁剪（基于路径前缀和 TTL）。禁止承载：通用语义理解、跨任务规划、生成新记忆内容。三类操作若需要，归属外部应用层。推理皮层物理驻留 WM（注意力调度器在图中标注于 WM 层附近仅表示其与 WM 槽位容量的关联关系，不表示其物理驻留——调度器为独立服务，见 §9）（见 §0.4 六类结构原则说明） | 不持久化（临时隔离区） |
...
**认知机制→架构模式偏好声明**：认知层为每个核心认知机制指定架构模式偏好——(a) **注意力调度器** → 横切执行实体（集中式）；(b) **内隐调制** → 横切注入模式（分布在各层）；(c) **情感去强化 / VAD 提升** → 横切协调协议（由元认知层统一协调）；(d) **推理皮层** → 路由协调层（跨层协调，物理驻留 WM（注意力调度器在图中标注于 WM 层附近仅表示其与 WM 槽位容量的关联关系，不表示其物理驻留——调度器为独立服务，见 §9） 但功能独立）；(e) **元记忆** → 元认知层内部子域；(f) **前瞻保持** → 跨层协调协议（三层协同）。以上偏好不约束具体实现细节，但约束组件的架构归属。
...
> 此重构消除了原「操作真理↔规范真理双轨切换」的模式：身份不再通过「切换至规范真理模式」来介入，而是以否决查询的形式在每次排序产出中自动过审——从代价高昂的系统级模式切换降为细粒度的布尔否决查询。**可靠性注**：否决权依赖三个结构单元（元认知层→策略层→存储层）协同——当任一元降级时否决权可靠性随之降级。此成本是否可接受取决于否决权在运行中的调用频率（预期低频）；若高频调用，考虑将身份注册表本地缓存至策略层
...
### 1.7 监督平面（已合并入外部治理接口——本节保留为监督职能历史记录）（Supervisory Plane）
...
策略层
  ...
  │   **外部校准源充分性建模**（v1.0 设计框架占位（v1.1 实现为系统一等公民），认知基础§2.1 他律性悖论的架构承载）：校准源不是无条件可信的外部给定——本协议扩展以下评估维度以逼近校准源充分性：
...
- **注意力调度器的跨层接口要求**：注意力调度器为独立横切服务（不驻留 WM 层）（以便与 WM 槽位容量直接关联），但其调度范围覆盖编码、巩固、检索三个环节——即存储层的编码预算（§10.5 摄取门禁）、存储层的巩固资源（§4 巩固子模块间隔系数调制）、策略层的候选裁剪（§3.2 预测器）均需向注意力调度器暴露接口以供查询当前注意力预算可用性。各层不持有独立的注意力预算——注意力是全局横切资源，各层仅可在当前全局预算约束下请求分配。此约束的目的在于防止存储层在巩固环节消耗超出全局可用的注意力资源，与注意力是编码上游前提（认知基础 §1.3）的理念一致。当某层请求的注意力预算超过当前可用配额时，注意力调度器返回降级信号——该层须执行降级操作（如降低巩固深度、缩窄检索候选集）。

**注意力调度器的归类声明**（定义详见 §9.2）：注意力调度器虽是全局横切资源管理器（横切调制逻辑），但因其执行实体须与 WM 槽位容量直接关联而物理驻留 WM（注意力调度器在图中标注于 WM 层附近仅表示其与 WM 槽位容量的关联关系，不表示其物理驻留——调度器为独立服务，见 §9） 层。此归属不改变其横切性质——注意力是横切属性的执行实体，而非 WM 层的私有能力。归类上属横切属性且执行实体驻留某一层，与内隐调制基元的跨层分布逻辑一致。
...
**准见证锚定映射声明**（认知基础 §三 受限自主阶段认知模型的架构映射；**v1.1+ 扩展目标**）：认知基础定义了受限自主阶段的「准见证锚定」三阶模型（三级递进：短期静默维持现有锚定→中期启用交叉验证→长期安全休眠），以及准见证锚定的充分性条件（≥3 种记忆类型互证、无单类型被使用权重扭曲、各类型≥2 次独立检索记录）。架构 §10.9 降级状态机的 `provisional_*` 标记与 `pending_constitutional_review` 状态是此三阶模型的工程代理，但存在以下映射缺口——(a) 三级递进的触发阈值（N/M 周期）在架构中由 `DEGRADATION_PERIOD_N`/`DEGRADATION_PERIOD_M` 参数承载，但认知层定义的三阶递进条件是连续的阈值区间而非离散跳变；(b) 准见证锚定的三条充分性条件在架构中没有独立的检测逻辑——当前降级状态机仅以校准时延为唯一触发输入，未纳入记忆类型互证和检索独立性的判定。此映射缺口已登记于差距表 G-02，v1.1 目标将三条充分性条件与降级状态机的触发逻辑绑定。
...
| **推理皮层** | §4——独立协调层（物理驻留 WM（注意力调度器在图中标注于 WM 层附近仅表示其与 WM 槽位容量的关联关系，不表示其物理驻留——调度器为独立服务，见 §9）），常设最小推理内核。管理 session_task_id 生命周期，承载前瞻保持触发监控/使用事件优先级排序/候选集上下文裁剪三类操作 |
