...
| 术语 | 英文 | 定义 | 来源 |
|:----|:-----|:-----|:-----|
| 外部治理接口 | External Governance Interface | 接收外部校准信号、冻结/解冻指令，输出审计日志。不持有至高否决权——策略层查询其状态后自行决策。由原宪法主权面与监督平面合并（见架构 §1.7） | 架构 §1.7 |
| 身份面 | Identity Plane | 独立正交治理面，以否决权而非排名介入辞典式排序。身份不在此链中（否决查询而非排序），与宪法主权面并列（外部安全高于内部同一性） | 认知基础 §2.1 / 架构 §1 |
