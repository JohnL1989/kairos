...
| 元认知层 | Metacognition Layer | 监控、评估、调节下层行为。含检测器族（耦合计监测器、VAD 独立性测试器）、治理器族、自观察记忆 | 架构 §2 |
...
| 监督平面 | Supervision Plane | 独立审计与证伪监视器——审计庭、VAD 独立性测试器、体系聚合审计器 | 架构 §1.7 |
...
| 辞典式排序 | Lexicographic Ordering | 六级辞典式排序链（探索>宪法>校准>认知完整性>时间>间接度）+ 身份面否决权，宪法级不变量 | 架构 §3.2 |
