...
本文的五维加权系数模型与 `value-dimension-entropy.md` 的熵值检测共享同一个五维使用空间，但回答不同问题：

- **本文（加权系数）**：各维度对升华/遗忘优先级的相对重要性——系数越高，该维度的负载对决策的影响越大
- **熵值检测**：当前使用分布是否异常坍缩——以等概率分布为参考基线判断多样性

两者不是竞争关系：加权系数设定「什么重要」，熵值检测回答「当前的分布是否健康」。同一五维向量的两种不同用途
...
| 版本 | 日期 | 变更 |
|:----|:----|:-----|
| v1.0.0 | 2026-07-19 | 初始算法。使用负载五维系数 + 组合记忆/遗忘去语境化/升华优先级标量化规则。 |
