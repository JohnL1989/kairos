...
@router.post("/archive")
async def archive_tool_call(req: ToolArchiveRequest, user_id: str = Depends(get_current_user),
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            ...
            req.project_id, req.duration_ms, user_id
        )
        ...
        try:
            ...
            mem_id = None  # skip memory write if fails
...
@router.get("/skills")
async def list_skills(limit: int = 20, user_id: str = Depends(get_current_user),
                      pool: asyncpg.Pool = Depends(get_pool)) -> dict:
    """查询沉淀的技能"""
    async with pool.acquire() as conn:
        rows = await conn.fetch(
