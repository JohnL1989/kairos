...
async def chunk_memory(pool: asyncpg.Pool, memory_id: int, embed_fn: Callable) -> dict:
    ...
    async with pool.acquire() as conn:
        ...
        try:
            ...
            log.warning("chunk_memory: embedding 生成失败，分块将不带向量（embedding=NULL）")

        now = datetime.now(timezone.utc)
        for i, ct in enumerate(chunk_texts):
            vec_str = None
            if embeddings is not None and i < len(embeddings) and embeddings[i] and len(embeddings[i]) > 0:
                vec_str = vec_to_str(embeddings[i])
            await conn.execute(
...
async def chunk_all_unprocessed(pool: asyncpg.Pool, user_id: str, embed_fn: Callable, batch_size: int = 20) -> dict:
    async with pool.acquire() as conn:
